/* components/heatmap-grid.js — <door-heatmap> (Fig. 5)
 *
 * Open-probability heatmap, hour (x) × weekday (y), drawn as a DOM grid of
 * stippled squares. Colour is a printed ink-density scale: pale cream → deep
 * green ink. Logic-only: update(heat).
 */
import { BaseFigure } from "./base-figure.js";
import { WD, WD_LONG } from "../core/format.js";

/* pale cream(246,241,228) → green ink(31,122,82) by open share */
...
export class HeatmapGrid extends BaseFigure {
  ...
  afterRender() {
    // static legend swatches
    this.$("[data-scale]").innerHTML = [0, 20, 40, 60, 80, 100].map((p) =>
      `<i class="w-[13px] h-[13px] rounded-[2px] inline-block" style="background:${heatColor(p)}"></i>`).join("");
  }
